    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <TrustCard
          ...
          body="IndexedDB keeps local summaries, settings, calibration profiles, and posture/reminder events. Raw camera frames and video recordings are not part of the storage model."
        />
        <TrustCard
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <article className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 font-display text-2xl text-white">What this browser is storing right now</h2>
            </div>
            {privacy.isLoading ? <p className="text-sm text-slate-400">Reading local storage...</p> : null}
          </div>

          <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            <MetricCard label="Settings records" value={String(snapshot?.settingsCount ?? 0)} />
            <MetricCard label="Calibration profiles" value={String(snapshot?.calibrationProfilesCount ?? 0)} />
            <MetricCard label="Sessions" value={String(snapshot?.sessionsCount ?? 0)} />
            <MetricCard label="Completed sessions" value={String(snapshot?.completedSessionsCount ?? 0)} />
            <MetricCard label="Daily summaries" value={String(snapshot?.dailyMetricsCount ?? 0)} />
            <MetricCard label="Stored events" value={String(snapshot?.eventsCount ?? 0)} />
          </div>

          <div className="mt-6 grid gap-3 md:grid-cols-2">
            <DetailCard
              label="Latest completed session"
              value={snapshot?.latestCompletedSessionAt ? formatDateTime(snapshot.latestCompletedSessionAt) : 'None yet'}
            />
            <DetailCard
              label="Latest stored event"
              value={snapshot?.latestEventAt ? formatDateTime(snapshot.latestEventAt) : 'None yet'}
            />
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-slate-950/55 p-5 text-sm leading-6 text-slate-300">
            ...
            <p className="mt-3">
              The app stores posture summaries, settings, calibration, sessions, and reminder/event history locally. Raw webcam footage is not persisted by this implementation.
            </p>
          </div>
        </article>
        ...
        <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-panel">
          ...
          <h2 className="mt-2 font-display text-2xl text-white">Offline and install readiness</h2>

          <div className="mt-6 grid gap-3 md:grid-cols-2">
            <StatusCard label="Connection" value={privacy.runtimeStatus.isOnline ? 'Online' : 'Offline'} />
            <StatusCard label="Installed mode" value={privacy.runtimeStatus.isStandalone ? 'Standalone' : 'Browser tab'} />
            <StatusCard
              label="Service worker"
              value={privacy.runtimeStatus.serviceWorkerSupported ? 'Supported' : 'Unsupported'}
            />
            <StatusCard
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5 text-sm leading-6 text-slate-300">
            ...
            <p className="mt-2 text-slate-400">
              Offline-capable installation and local-only storage remain part of the app promise. This panel makes that runtime state visible instead of implicit.
            </p>
          </div>
        </article>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
          ...
          <h2 className="mt-2 font-display text-2xl text-white">Export, reset, and clear local data</h2>
          ...
          <div className="mt-6 space-y-3">
            <ActionRow
              ...
              buttonLabel={privacy.activeAction === 'export' ? 'Exporting...' : 'Export JSON'}
            ...
            <ActionRow
              ...
              buttonLabel={privacy.activeAction === 'reset-calibration' ? 'Resetting...' : 'Reset calibration'}
              buttonClassName="border border-white/10 text-white hover:bg-white/10"
              disabled={privacy.activeAction !== null}
              onClick={() => {
                if (!window.confirm('Reset saved calibration data and keep the rest of the local history?')) {
                  return;
                }
            ...
            <ActionRow
              ...
              buttonLabel={privacy.activeAction === 'reset-settings' ? 'Resetting...' : 'Reset settings'}
              buttonClassName="border border-white/10 text-white hover:bg-white/10"
              disabled={privacy.activeAction !== null}
              onClick={() => {
                if (!window.confirm('Reset saved settings to their default values?')) {
                  return;
                }
            ...
            <ActionRow
              title="Clear history"
              body="Delete saved sessions, daily summaries, events, and transient session samples while keeping current settings and calibration profiles."
              buttonLabel={privacy.activeAction === 'clear-history' ? 'Clearing...' : 'Clear history'}
              buttonClassName="border border-amber-400/30 text-amber-100 hover:bg-amber-400/10"
              disabled={privacy.activeAction !== null}
              onClick={() => {
                if (!window.confirm('Clear local history but keep settings and calibration?')) {
                  return;
                }
            ...
            <ActionRow
              ...
              body="Delete every local record in this browser, including settings, calibration, sessions, rollups, and event history. The app will fall back to default settings afterward."
              buttonLabel={privacy.activeAction === 'clear-all' ? 'Clearing...' : 'Clear everything'}
              buttonClassName="border border-rose-400/30 text-rose-100 hover:bg-rose-400/10"
              disabled={privacy.activeAction !== null}
              onClick={() => {
                if (!window.confirm('Clear every local record in this browser? This cannot be undone.')) {
                  return;
                }
        ...
        <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-panel">
          ...
          <h2 className="mt-2 font-display text-2xl text-white">Common privacy-friendly workflows</h2>

          <div className="mt-6 space-y-4 text-sm leading-6 text-slate-300">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="font-semibold text-white">Share the project with another developer</p>
              <p className="mt-2">Export the local JSON first, then hand off the codebase and the status document for implementation context.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="font-semibold text-white">Recalibrate after changing your workspace</p>
              <p className="mt-2">Use onboarding again if your camera angle, chair, or desk setup changes enough to shift the baseline.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="font-semibold text-white">Start over without losing trust</p>
              <p className="mt-2">You can clear only history, only calibration, or everything, instead of treating all data reset needs as one destructive action.</p>
            </div>
          </div>
...
function ActionRow({ title, body, buttonLabel, buttonClassName, disabled, onClick }: ActionRowProps) {
  return (
