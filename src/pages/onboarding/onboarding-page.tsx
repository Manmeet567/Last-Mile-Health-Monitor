...
export function OnboardingPage() {
  ...
  const [captureState, setCaptureState] = useState<'idle' | 'capturing' | 'saving' | 'completed' | 'error'>('idle');
  ...
  const isCaptureReady = Boolean(
    processedPose?.features.isConfidenceSufficient &&
      ...
      processedPose.features.shoulderTiltDeg !== null,
  );

  useEffect(() => {
    if (captureState !== 'capturing' || !isCaptureReady || !processedPose?.normalizedPose) {
      return;
    }
    ...
    setSamples((currentSamples) => {
      ...
      return [
        ...currentSamples,
        {
          ...
          shoulderTiltDeg: processedPose.features.shoulderTiltDeg ?? 0,
  ...
  useEffect(() => {
    if (captureState !== 'capturing' || samples.length < CALIBRATION_TARGET_SAMPLE_COUNT) {
      return;
    }
    ...
    async function completeCalibration() {
      ...
      try {
        ...
        setMessage('Calibration complete. Your local posture baseline is saved.');
      } catch (error) {
        if (cancelled) {
          return;
        }
        ...
        setMessage(error instanceof Error ? error.message : 'Calibration could not be completed.');
      }
    }
  ...
  const progressPercentage = Math.min((samples.length / CALIBRATION_TARGET_SAMPLE_COUNT) * 100, 100);
      ...
      <section className="grid gap-4 xl:grid-cols-[1.4fr_0.9fr]">
        ...
        <div className="space-y-4">
          <article className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
            <h2 className="font-display text-xl text-white">Calibration checklist</h2>
            <ol className="mt-4 space-y-3 text-sm leading-6 text-slate-300">
              <li>Start the camera and sit in your natural upright desk posture.</li>
              <li>Keep your shoulders and hips visible until the pose becomes reliable.</li>
              <li>Run the capture for a short stable sample so the app can build your baseline.</li>
            </ol>
            ...
            <div className="mt-6 space-y-4">
              <label className="block space-y-2">
                ...
                <select
                  ...
                  onChange={(event) => {
                    setPreferredSensitivity(event.target.value as CalibrationProfile['preferredSensitivity']);
                  }}
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  ...
                  disabled={!streamReady || captureState === 'capturing' || captureState === 'saving'}
                >
                  Start calibration
                </button>
                <button
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-300">
                  {samples.length} / {CALIBRATION_TARGET_SAMPLE_COUNT} stable samples collected
                </p>
              </div>

              <p className="text-sm text-slate-300">
                Pose ready for calibration:{' '}
                <span className="font-semibold text-white">{isCaptureReady ? 'yes' : 'not yet'}</span>
              </p>
          ...
          <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-panel">
            ...
            <dl className="mt-4 space-y-3 text-sm text-slate-300">
              ...
              <MetricRow label="Worker status" value={poseStream.workerState.phase} />
              <MetricRow
      ...
      <section className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <article className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-panel backdrop-blur">
          ...
          {calibration.isLoading ? (
            <p className="mt-4 text-sm text-slate-300">Loading saved calibration...</p>
          ) : calibration.profile ? (
            <dl className="mt-4 space-y-3 text-sm text-slate-300">
              <MetricRow label="Last updated" value={new Date(calibration.profile.updatedAt).toLocaleString()} />
              ...
              <MetricRow
                label="Baseline shoulder tilt"
                value={`${calibration.profile.baselineShoulderLevelDelta.toFixed(2)} deg`}
              />
              <div className="pt-3">
                <button
                  type="button"
            ...
            <p className="mt-4 text-sm leading-6 text-slate-300">
              No saved calibration yet. Complete one short capture to create your personal baseline.
            </p>
          )}
        </article>

        <article className="rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-panel">
          <h2 className="font-display text-xl text-white">Derived thresholds</h2>
          {calibration.profile && calibrationSummary ? (
            ...
            <p className="mt-4 text-sm leading-6 text-slate-300">
              Derived thresholds will appear here after calibration is saved locally.
            </p>
          )}
        </article>
      </section>
  ...
  function startCapture() {
    ...
    setMessage('Hold still in your natural upright posture while samples are collected.');
  }

  function resetCapture(nextState: 'idle' | 'completed' | 'error', nextMessage: string | null) {
    lastCapturedTimestampRef.current = null;
    setSamples([]);
